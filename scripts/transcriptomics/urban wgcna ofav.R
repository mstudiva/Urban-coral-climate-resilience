...
lnames=load("../../outputs/transcriptomics/ofav/data4wgcna.RData")
...
pdf("../../outputs/transcriptomics/ofav/sample mean expression.pdf",height=4, width=8)
barplot(meanExpressionByArray,
        xlab = "Sample", ylab = "Mean expression",
        main ="Mean expression across samples",
        names.arg = c(1:66), cex.names = 0.7)
dev.off()
...
write.csv(traits, file="../../outputs/transcriptomics/ofav/traits.csv")

save(datt,traits,file="../../outputs/transcriptomics/ofav/wgcnaData.RData")
...
options(stringsAsFactors=FALSE)
allowWGCNAThreads()
load("../../outputs/transcriptomics/ofav/wgcnaData.RData")
...
sizeGrWindow(9, 5)
pdf("../../outputs/transcriptomics/ofav/soft threshold signed.pdf",height=4, width=8)
...
s.th=8 # re-specify according to previous section
...
save(dynamicMods,dynamicColors,MEs,METree,geneTree,file="../../outputs/transcriptomics/ofav/1stPassModules.RData")


#### MERGING MODULES ####

mm=load('../../outputs/transcriptomics/ofav/1stPassModules.RData')
mm
lnames=load('../../outputs/transcriptomics/ofav/wgcnaData.RData')
# traits
# head(datt)

quartz()

MEDissThres = 0.25 # in the first pass, set this to 0 - no merging (we want to see the module-traits heatmap first, then decide which modules are telling us the same story and better be merged)
sizeGrWindow(7, 6)
...
save(MEs, geneTree, moduleLabels, moduleColors, file = "../../outputs/transcriptomics/ofav/networkdata_signed.RData")


#### MODULE CORRELATIONS ####
# plotting correlations with traits:
load(file = "../../outputs/transcriptomics/ofav/networkdata_signed.RData")
load(file = "../../outputs/transcriptomics/ofav/wgcnaData.RData");
...
labeledHeatmap(Matrix = moduleTraitCor,
               ...
               cex.text = 0.7,
               zlim = c(-0.7,0.7),
               main = paste("O. faveolata Module-Trait correlations"))

# module size barplot
labelShift=500 # increase to move module size labels to the right
quartz()
par(mar = c(6, 8.5, 3, 3));
mct=table(moduleColors)
mct[modLabels]
x=barplot(mct[rev(modLabels)],horiz=T,las=1,xlim=c(0,6000),col=rev(modLabels))
...
# moduleCols=c("purple") # for lowpH
moduleCols=c("purple","black","midnightblue","brown") # for hightemp
# moduleCols=c("magenta","cyan","black") # for genotype
# moduleCols=c("magenta","cyan","black") # for growthrate
# moduleCols=c("magenta","cyan","black") # for fvfm
# moduleCols=c("magenta","cyan","black") # for fvfm_ratio
# moduleCols=c("magenta","cyan","black") # for survival
# moduleCols=c("magenta","cyan") # for propB
# moduleCols=c("magenta","cyan","black") # for propD
# moduleCols=c("purple","black","midnightblue","brown") # for temp_actual
# moduleCols=c("black","midnightblue") # for ph_actual
...
# par(mfrow=c(1,1)) # for lowpH
par(mfrow=c(1,4)) # for hightemp
# par(mfrow=c(1,3)) # for genotype
# par(mfrow=c(1,3)) # for growthrate
# par(mfrow=c(1,3)) # for fvfm
# par(mfrow=c(1,3)) # for fvfm_ratio
# par(mfrow=c(1,3)) # for survival
# par(mfrow=c(1,2)) # for propB
# par(mfrow=c(1,3)) # for propD
# par(mfrow=c(1,4)) # for temp_actual
# par(mfrow=c(1,2)) # for ph_actual
...
library(WGCNA)
load(file = "../../outputs/transcriptomics/ofav/networkdata_signed.RData") # moduleColors, MEs
load(file = "../../outputs/transcriptomics/ofav/wgcnaData.RData") # vsd table
load(file = "../../outputs/transcriptomics/ofav/data4wgcna.RData") # vsd table
...
library(WGCNA)
load(file = "../../outputs/transcriptomics/ofav/networkdata_signed.RData")
load(file = "../../outputs/transcriptomics/ofav/data4wgcna.RData") 
load(file = "../../outputs/transcriptomics/ofav/wgcnaData.RData");
allkME =as.data.frame(signedKME(datt, MEs))
gg=read.delim(file="../../../Orbicella-faveolata-annotated-transcriptome/Ofaveolata_iso2geneName.tab",sep="\t")
library(pheatmap)
...
# pdf(file="heatmap top30 magenta.pdf", height=6, width=18)
# pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F,cluster_cols=F)
# dev.off()
# pdf(file="heatmap top30 purple.pdf", height=6, width=18)
# pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F,cluster_cols=F)
# dev.off()
# pdf(file="heatmap top30 cyan.pdf", height=6, width=16)
# pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F,cluster_cols=F)
# dev.off()
# pdf(file="heatmap top30 black.pdf", height=6, width=18)
# pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F,cluster_cols=F)
# dev.off()
# pdf(file="heatmap top30 midnightblue.pdf", height=6, width=16)
# pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F,cluster_cols=F)
# dev.off()
pdf(file="../../outputs/transcriptomics/ofav/heatmap top30 brown.pdf", height=6, width=18)
pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F,cluster_cols=F)
dev.off()
...
write.csv(hubgenes, file="../../outputs/transcriptomics/ofav/hubgenes.csv")
