...
lnames=load("../../outputs/transcriptomics/ssid/data4wgcna.RData")
...
pdf("../../outputs/transcriptomics/ssid/sample mean expression.pdf",height=4, width=8)
...
write.csv(traits, file="../../outputs/transcriptomics/ssid/traits.csv")

save(datt,traits,file="../../outputs/transcriptomics/ssid/wgcnaData.RData")
...
options(stringsAsFactors=FALSE)
allowWGCNAThreads()
load("../../outputs/transcriptomics/ssid/wgcnaData.RData")
...
sizeGrWindow(9, 5)
pdf("../../outputs/transcriptomics/ssid/soft threshold signed.pdf",height=4, width=8)
...
save(dynamicMods,dynamicColors,MEs,METree,geneTree,file="../../outputs/transcriptomics/ssid/1stPassModules.RData")


#### MERGING MODULES ####

mm=load('../../outputs/transcriptomics/ssid/1stPassModules.RData')
mm
lnames=load('../../outputs/transcriptomics/ssid/wgcnaData.RData')
# traits
# head(datt)
...
save(MEs, geneTree, moduleLabels, moduleColors, file = "../../outputs/transcriptomics/ssid/networkdata_signed.RData")


#### MODULE CORRELATIONS ####
# plotting correlations with traits:
load(file = "../../outputs/transcriptomics/ssid/networkdata_signed.RData")
load(file = "../../outputs/transcriptomics/ssid/wgcnaData.RData");
...
mct[modLabels]
x=barplot(mct[rev(modLabels)],horiz=T,las=1,xlim=c(0,15000),col=rev(modLabels))
...
moduleCols=c("turquoise","blue") # for hightemp and temp_actual
...
par(mfrow=c(1,2)) # for hightemp and temp_actual
...
library(WGCNA)
load(file = "../../outputs/transcriptomics/ssid/networkdata_signed.RData") # moduleColors, MEs
load(file = "../../outputs/transcriptomics/ssid/wgcnaData.RData") # vsd table
load(file = "../../outputs/transcriptomics/ssid/data4wgcna.RData") # vsd table
...
library(WGCNA)
load(file = "../../outputs/transcriptomics/ssid/networkdata_signed.RData")
load(file = "../../outputs/transcriptomics/ssid/data4wgcna.RData") 
load(file = "../../outputs/transcriptomics/ssid/wgcnaData.RData");
allkME =as.data.frame(signedKME(datt, MEs))
gg=read.delim(file="../../../Siderastrea-siderea-annotated-transcriptome/Ssiderea_iso2geneName.tab",sep="\t")
library(pheatmap)
...
# dev.off()
pdf(file="../../outputs/transcriptomics/ssid/heatmap top30 blue.pdf", height=6, width=18)
pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F,cluster_cols=F)
dev.off()
...
hubgenes %>%
  rename("gene" = 
           hubgenes) %>%
  left_join(read.table(file = "../../../Siderastrea-siderea-annotated-transcriptome/Siderastrea_iso2geneName.tab",
                       sep = "\t",
                       quote="", fill=FALSE) %>%
              mutate(gene = V1,
                     annot_mcav = V2) %>%
...
write.csv(hubgenes, file="../../outputs/transcriptomics/ssid/hubgenes.csv")
