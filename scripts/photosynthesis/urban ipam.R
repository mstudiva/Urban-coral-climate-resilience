...
ipam <- read.csv(file="../../data/photosynthesis/urban ipam.csv", head=T)
str(ipam)

# changing genotype numbers to be distinct across sites/species
genotypes <- read.csv(file="../../data/genotype lookup table.csv", head=T)
genotypes
...
pdf("../../outputs/photosynthesis/urban ipam ssid normality.pdf")
ggscatter(
...
capture.output(get_anova_table(ofav_aov), file = "../../outputs/photosynthesis/urban ipam ofav ancova.txt")
...
capture.output(get_anova_table(ofav_control_aov), file = "../../outputs/photosynthesis/urban ipam ofav controls ancova.txt")

ssid_control_aov <- ipam_ssid_control %>% 
  anova_test(ratio ~ time + treatment*site)
get_anova_table(ssid_control_aov) # time and treatment significant
capture.output(get_anova_table(ssid_control_aov), file = "../../outputs/photosynthesis/urban ipam ssid controls ancova.txt")
...
capture.output(get_anova_table(ofav_heat_aov), file = "../../outputs/photosynthesis/urban ipam ofav heat ancova.txt")

ssid_heat_aov <- ipam_ssid_heat %>% 
  anova_test(ratio ~ time + treatment*site)
get_anova_table(ssid_heat_aov) # time, treatment, and site significant
capture.output(get_anova_table(ssid_heat_aov), file = "../../outputs/photosynthesis/urban ipam ssid heat ancova.txt")
...
ggsave("../../outputs/photosynthesis/urban ipam ofav.pdf", ofav, width=12, height=6,dpi = 300)
...
ggsave("../../outputs/photosynthesis/urban ipam ssid.pdf", ssid, width=12, height=6,dpi = 300)
