...
#read in counts
counts = read.table("../../../../raw/ofav/allcounts_sym2.txt")
...
pdf(file="heatmap_ofav_sym2.pdf", width=15, height=15)
pheatmap(cor(vsd))
dev.off()
...
pdf(file="PCoA_ofav_sym2.pdf", width=12, height=6)
par(mfrow=c(1,2))
plot(scores[,1], scores[,2],col=c("#018571","#80cdc1","#dfc27d","#a6611a")[as.numeric(as.factor(conditions$site))],pch=c(6,2,25,17)[as.numeric(as.factor(conditions$treat))], bg= c("#018571","#80cdc1","#dfc27d","#a6611a")[as.numeric(as.factor(conditions$site))], xlab="Coordinate 1 (91.8%)", ylab="Coordinate 2 (2.2%)", main="Site")
...
plot(scores[,1], scores[,2],col=c("#92c5de","#f4a582","#0571b0","#ca0020")[as.numeric(as.factor(conditions$treat))],pch=c(15,0,1,16)[as.numeric((as.factor(conditions$site)))], xlab="Coordinate 1 (91.8%)", ylab="Coordinate 2 (2.2%)", main="Treatment")
...
# p < 1e-12
# pdf(file="heatmap_site_p1e-12.pdf", height=24, width=40)
...
# )
# dev.off()
# Too many DEGs passing a p < 1e-12 filter!
