...
lnames=load("../../../DESeq2/ofav/sym/data4wgcna.RData")
...
# module size barplot
labelShift=1500 # increase to move module size labels to the right
quartz()
par(mar = c(6, 8.5, 3, 3));
...
# moduleCols=c("turquoise","blue","brown") # for Emerald
# moduleCols=c("turquoise","blue","brown") # for Star
# moduleCols=c("turquoise","blue","brown") # for MacN
# moduleCols=c("blue") # for hightemp
# moduleCols=c("turquoise","blue","brown") # for genotype
# moduleCols=c("turquoise","blue","brown") # for growthrate
# moduleCols=c("turquoise","blue","brown") # for fvfm
# moduleCols=c("turquoise","blue","brown") # for fvfm_ratio
# moduleCols=c("turquoise","blue","brown") # for survival
# moduleCols=c("turquoise","blue","brown") # for propB
# moduleCols=c("turquoise","blue","brown") # for propD
moduleCols=c("blue") # for temp_actual
...
# par(mfrow=c(1,3)) # for Emerald
# par(mfrow=c(1,3)) # for Star
# par(mfrow=c(1,3)) # for MacN
# par(mfrow=c(1,1)) # for hightemp
...
# par(mfrow=c(1,3)) # for propB
# par(mfrow=c(1,3)) # for propD
par(mfrow=c(1,1)) # for temp_actual
...
load(file = "../../../DESeq2/ofav/sym/data4wgcna.RData") # vsd table
...
library(WGCNA)
load(file = "networkdata_signed.RData")
load(file = "../../../DESeq2/ofav/sym/data4wgcna.RData") 
load(file = "wgcnaData.RData");
allkME =as.data.frame(signedKME(datt, MEs))
...
# dev.off()
# pdf(file="heatmap top30 blue.pdf", height=6, width=24)
# pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F,cluster_cols=F)
# dev.off()
pdf(file="heatmap top30 brown.pdf", height=6, width=39)
pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F,cluster_cols=F)
dev.off()
