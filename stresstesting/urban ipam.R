...
ipam <- read.csv(file="ipam.csv", head=T)
str(ipam)

# coding factors
ipam$genotype <- as.factor(ipam$genotype)
ipam$replicate <- as.factor(ipam$replicate)
ipam$tank <- as.factor(ipam$tank)
ipam$AOI <- as.factor(ipam$AOI)
...
get_anova_table(ofav_aov)
# all factors and treatment:site interaction significant

ssid_aov <- ipam_ssid_sub %>% 
  anova_test(ratio ~ time + treatment*site)
get_anova_table(ssid_aov)
# all factors and treatment:site interaction significant
...
ofav_control_aov <- ipam_ofav_control %>% 
  anova_test(ratio ~ time + treatment*site)
get_anova_table(ofav_control_aov)
# all factors and treatment:site interaction significant

ssid_control_aov <- ipam_ssid_control %>% 
  anova_test(ratio ~ time + treatment*site)
get_anova_table(ssid_control_aov)
# time and treatment significant
...
ofav_heat_aov <- ipam_ofav_heat %>% 
  anova_test(ratio ~ time + treatment*site)
get_anova_table(ofav_heat_aov)
# all factors and treatment:site interaction significant

ssid_heat_aov <- ipam_ssid_heat %>% 
  anova_test(ratio ~ time + treatment*site)
get_anova_table(ssid_heat_aov)
# time and treatment significant
...
ofav_control_time <- data.frame(time = 10,ratio = 1,lab = "time: F1,567=136.8, p<0.001*", pH = "low pH", temp="control temp")
ofav_control_treat <- data.frame(time = 10,ratio = 1,lab = "treat: F3,567=14.8, p<0.001*", pH = "low pH", temp="control temp")
ofav_control_site <- data.frame(time = 10,ratio = 1,lab = "site: F3,567=134.8, p<0.001*", pH = "low pH", temp="control temp")
ofav_control_int <- data.frame(time = 10,ratio = 1,lab = "int: F9,567=3.4, p<0.001*", pH = "low pH", temp="control temp")
...
# creating dummy variables for stats labels on plots
ssid_control_time <- data.frame(time = 10,ratio = 1,lab = "time: F1,651=20.9, p<0.001*", pH = "low pH", temp="control temp")
ssid_control_treat <- data.frame(time = 10,ratio = 1,lab = "treat: F3,651=88.1, p<0.001*", pH = "low pH", temp="control temp")
ssid_control_site <- data.frame(time = 10,ratio = 1,lab = "site: F3,651=1.6, p=0.19", pH = "low pH", temp="control temp")
ssid_heat_time <- data.frame(time = 10,ratio = 1,lab = "time: F1,703=7.2, p=0.007*", pH = "low pH", temp="high temp")
ssid_heat_treat <- data.frame(time = 10,ratio = 1,lab = "treat: F1,703=77.3, p<0.001*", pH = "low pH", temp="high temp")
ssid_heat_site <- data.frame(time = 10,ratio = 1,lab = "site: F3,703=5.5, p=0.001*", pH = "low pH", temp="high temp")
